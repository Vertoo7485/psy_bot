module Telegram
  module Handlers
    module SelfHelpHandlers
      class Day28Handler < BaseHandler
        def process
          log_info("Processing day 28 callback: #{@callback_data} - User: #{@user.telegram_id}, Chat: #{@chat_id}")
          
          begin
            day_service = SelfHelp::Days::Day28Service.new(bot_service, user, chat_id)
            day_service.handle_button(@callback_data)
            
          rescue => e
            log_error("Error in Day28Service processing", e)
            send_message(
              text: "Произошла ошибка при обработке финального дня. Пожалуйста, попробуйте еще раз."
            )
            answer_callback_query( "❌ Ошибка")
            return
          end
          
          answer_callback_query( "✅")
        end
        
        private
        
        def send_message(text:, reply_markup: nil, parse_mode: nil)
          @bot_service.send_message(
            chat_id: @chat_id,
            text: text,
            reply_markup: reply_markup,
            parse_mode: parse_mode
          )
        rescue => e
          log_error("Failed to send message", e)
        end
      end
    end
  end
end