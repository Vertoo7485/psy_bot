  ...
  def can_start_day_program?(day_number)
    ...
    if current_day_started_at
      ...
      if time_passed < 12.hours
        ...
        hours_left_int = ((12.hours - time_passed) / 1.hour).floor
        minutes_left = (((12.hours - time_passed) % 1.hour) / 1.minute).ceil
        
        if hours_left_int > 0
          error_msg = "⏱️ До начала следующего дня осталось: #{hours_left_int} часов #{minutes_left} минут\n\nНовый день откроется через указанное время."
        else
          error_msg = "⏱️ До начала следующего дня осталось: #{minutes_left} минут\n\nНовый день откроется через указанное время."
        end
        errors << error_msg
